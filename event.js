...
function bind(curEle, evenType, evenFn) {
    ...
    //->首先判断该自定义属性之前是否存在,不存在话创建一个,由于要存储多个方法化妆后
    // 的结果,所以我们让其值是一个数组
    if (!curEle["myBind" + evenType]) {
        curEle["myBind" + evenType] = [];
    }
    //->解决重复问题：每一次自己在往自定义属性对应的容器中添加前，看一下之前是否已
    // 经有了，有的话就不用在重新的添加了，同理也不需要往事件池中存储了
...
function unbind(curEle, evenType, evenFn) {
    ...
    //->拿evenFn到curEle["myBind"]这里找化妆后的结果,
    // 找到之后在事件池中把换装后的结果给移出事件池
    var ary = curEle["myBind" + evenType];
    for (var i = 0; i < ary.length; i++) {
        var cur = ary[i];
        if (cur.photo === evenFn) {
...
function on(curEle, evenType, evenFn) {
    ...
    // ->执行on的时候,我们给当前元素绑定了一个点击的行为,
    // 当点击的时候执行run方法:run方法中的this是当前元素curEle,
    // 并且浏览器给run传递了一个MouseEvent事件对象
...
//->run:我们只给当前元素的点击行为绑定一个方法run,当触发点击的时候执行的是run方法,
// 我在run方法中根据自己存储的方法顺序分别的在把这些方法执行
function run(e) {
    e = e || window.event;
    var flag = e.target ? true : false;
    //->IE6~8下不兼容e.target,得到的flag为false
    if (!flag) {
